...
for n in range(75, 76):
    ...
    n_itr = n
    acc = 0
    while acc < math.ceil(N - (n - n_itr + 1)):
        acc += n_itr
        n_itr -= 1
    acc -= math.ceil(N - (n - n_itr + 1))
    #c_ = 1 - (acc / n)
    c_ = 0.5
    N1 = N - (n * (1 - c_))
    N2 = N + (n * (1 - c_))
    ...
    print(f"for n value {n}: maximum x: {guessed_x}, maximum y: {y_max - MARGIN}, estimated value: {Nceil}, difference: {max((guessed_y) - Nceil, (guessed_x - Nceil))}")
